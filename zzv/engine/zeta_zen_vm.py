...
from zzv.common.observability import setup_tracing

tracer = setup_tracing("zzv-engine")
...
class ZetaZenVm:
    def __init__(
        self, config=None, config_path=DEFAULT_CONFIG_PATH, additional_managers=None
    ):
        """
    ...
    def _setup_endpoints(self):
        ...
        async def start_all():
            try:
                ...
                raise HTTPException(
                    status_code=500, detail="Failed to start all services"
                )
        ...
        async def stop_all():
            try:
                ...
                raise HTTPException(
                    status_code=500, detail="Failed to stop all services"
                )
        ...
        async def start_service(service_name: str):
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to start service {service_name}"
                )
        ...
        async def stop_service(service_name: str):
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to stop service {service_name}"
                )
        ...
        async def health():
            try:
                ...
                raise HTTPException(
                    status_code=500, detail="Failed to retrieve health report"
                )
    ...
    async def run_async(self, host, port):
        ...
        config = uvicorn.Config(
            self.app, host=host, port=port, loop="asyncio", log_level="info"
        )
